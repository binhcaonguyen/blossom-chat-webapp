...
$('#upload-input').on('change', function(){
  ...
  if(uploadInput.val() != ''){
      ...
      message.text = link1;
      $('#messages').append('<li class="chatMessageRight">' + link1 + '</li>');
...
socket.on('newGroup', function(name){
    ...
    myGroups.forEach(function(group){
      ...
      groupsList += '<li id="' + group + '" class="' + activeClass + '" onclick="selectUerChatBox(this, \'' + group + '\', \'' + group + '\')"><a href="javascript:void(0)">' + group + '</a><label class="chatNotificationCount"></label></li>';
    });
